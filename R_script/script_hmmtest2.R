...
inits_stan=function(){
  ...
  list(mu_x=mu_x,ky=sigmay,kx=sigmax,sigbeta0=sigbeta0,sigbeta1=sigbeta1,
       ...
       beta1=beta1,beta0=beta0,betamu2=betamu2,PI=PI,rho=rho
  )
}
inits=list()
for(i in 1:3)inits[[i]] = inits_stan()

hmm.test<-stan(file="hmm_test2.stan",data=standata,chains=3,init=inits,iter=10000)

save(hmm.test,file="hmmtest.Rdata")
# 
