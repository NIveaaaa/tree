...
model {
  ...
  // log y[j,t]~N(beta0[j]+age[j,t]*beta1[j]+eta[t],sigmay^2)
  for (j in 1:nt){
    for(t in start[j]:end[j]){
      target += normal_lpdf(y[j,t] | beta0[j]+beta1[j]*age[j,t]+eta[t],sigmay);
    }
  }
  ...
  beta1~normal(betamu1+rho*sigbeta1/sigbeta0*(beta0-betamu0),sigbeta1*sqrt(1-rho*rho));
  ...
  for (t in (N_mis+1):(N_mis+42)){
    for (l in 1:len[t]){
      int ii=b[l,t];
      target+=normal_lpdf(y[ii,t]|beta0[ii]+beta1[ii]*age[ii,t]+eta_pred1[t-N_mis],sigmay);
    }
  }
  ...
  for (t in (N_mis+43):(N_mis+83)){
    for (l in 1:len[t]){
      int iii=b[l,t];
      target+=normal_lpdf(y[iii,t]|beta0[iii]+beta1[iii]*age[iii,t]+eta_pred2[t-N_mis-42],sigmay);
    }
  }
