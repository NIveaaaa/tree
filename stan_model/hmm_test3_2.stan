...
model {
  ...
  // log y[j,t]~N(betamu0+age[j,t]*beta1[j]+eta[t],sigmay^2)
  for (j in 1:nt){
    for(t in start[j]:end[j]){
      target += normal_lpdf(y[j,t] | betamu0+beta1[j]*age[j,t]+eta[t],sigmay);
    }
  }
  ...
  for (t in (N_mis+1):(N_mis+42)){
    for (l in 1:len[t]){
      int ii=b[l,t];
      target+=normal_lpdf(y[ii,t]|betamu0+beta1[ii]*age[ii,t]+eta_pred1[t-N_mis],sigmay);
    }
  }
  ...
  for (t in (N_mis+43):(N_mis+83)){
    for (l in 1:len[t]){
      int iii=b[l,t];
      target+=normal_lpdf(y[iii,t]|betamu0+beta1[iii]*age[iii,t]+eta_pred2[t-N_mis-42],sigmay);
    }
  }
